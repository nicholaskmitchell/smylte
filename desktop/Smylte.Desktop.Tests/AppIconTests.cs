...
public sealed class AppIconTests
{
    ...
    [Fact]
    public void Every_size_Windows_asks_for_is_present_exactly_once()
    {
        var sizes = Directory(Bytes()).Select(e => e.Size).ToList();
        Assert.Equal(Expected.Length, sizes.Count);
        Assert.Equal(Expected.Order(), sizes.Order());
    }

    [Fact]
    public void Entries_declare_themselves_as_square_32bpp_truecolour()
    {
        foreach (var e in Directory(Bytes()))
        {
    ...
    [Fact]
    public void Payloads_lie_inside_the_file_and_do_not_overlap()
    {
        var ico = Bytes();
        ...
        {
            Assert.True(e.Offset >= header, $"{e.Size}px payload overlaps the directory");
            Assert.True(e.Offset + e.Length <= ico.Length, $"{e.Size}px payload runs off the end");
        }
        for (int i = 1; i < spans.Count; i++)
        {
            Assert.True(spans[i].Offset >= spans[i - 1].Offset + spans[i - 1].Length,
                $"{spans[i].Size}px payload overlaps {spans[i - 1].Size}px");
        }
    }
    ...
    [Fact]
    public void Every_frame_is_a_non_interlaced_8_bit_RGBA_PNG_of_the_declared_size()
    {
        var ico = Bytes();
        foreach (var e in Directory(ico))
        {
            ...
            Assert.True(png[..8].SequenceEqual<byte>([0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A]),
                $"{e.Size}px payload is not a PNG");
    ...
    [Fact]
    public void Every_frame_has_transparent_pixels_rather_than_being_a_full_bleed_tile()
    {
        // Windows applies no mask, inset or rounding to a Win32 exe icon — what
        // the file holds is composited literally. An opaque rectangle is the one
        // thing no other icon in the taskbar is, and it is the whole complaint.
        var ico = Bytes();
        foreach (var e in Directory(ico))
        {
            ...
            Assert.True(clear > 0, $"{e.Size}px frame is fully opaque");
        }
    }

    [Fact]
    public void The_accent_period_survives_at_every_size()
    {
        ...
        foreach (var e in Directory(ico))
        {
            ...
            Assert.True(found, $"{e.Size}px frame has no opaque pixel near #C75A26");
        }
    }

    [Fact]
    public void Nothing_is_pure_black_or_pure_white()
    {
        var ico = Bytes();
        foreach (var e in Directory(ico))
        {
            ...
            for (int i = 0; i < px.Length; i += 4)
            {
                ...
                Assert.False(black || white, $"{e.Size}px frame contains pure black or white");
            }
        }
    }
