...
def build_tools(api) -> dict[str, Tool]:
    ...
    @tool(
        ...
        "mean nothing happened. `worked_minutes` is the one MEASURED number: what "
        "a focus session actually credited to the day's rows, and each entry "
        "carries its own `worked_seconds` beside its `estimate_minutes`.\n\n"
